...
## Router (Smart TVs, Apple TV, Gaming Consoles)
...
> **Warning:** Router-level filtering can be bypassed if a device uses its own DNS (like a VPN app on a phone). Use router filtering **in addition to** per-device protection, not instead of it.
...
Default login credentials (check your router's sticker if these don't work):
...
Or use the servers with your config ID embedded (more reliable):

| Setting | Value |
|---------|-------|
| Primary DNS | `45.90.28.X` |
| Secondary DNS | `45.90.30.X` |

Where `X` is calculated from your config ID. Find your exact IPs at:
[my.nextdns.io](https://my.nextdns.io) → **Setup** → scroll to **Routers** section

### Step 5: (Optional) Use DNS-over-HTTPS on Router
...
**Other routers:** Check if your router supports DoH/DoT. If not, standard DNS still works.
...
**Solution:** Block these IPs at the router firewall level:
- Block outbound traffic to `8.8.8.8`, `8.8.4.4` (Google DNS)
- Block outbound traffic to `1.1.1.1`, `1.0.0.1` (Cloudflare DNS)
- Block outbound UDP/TCP port 53 to anything except your router

This forces devices to use your router's DNS or fail.
